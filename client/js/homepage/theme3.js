...
// DPC: Replaced / fixed version for Falcon, originally written by lrowe
// DPC: Add external links to linkbox portlet links and teaser portlets
// DPC: Links to this.hostname shouldn't be marked as external regardless
projectlight.markExternalLinks = function(){
	var external_links;

	$('#content a, .falcon-linkbox-portlet a').
	not(".campl-carousel a").filter(function(){
		return(this.hostname &&
			this.hostname !== location.hostname &&
			!this.hostname.match(/\.cam\.ac\.uk$/gi));
	}).addClass("campl-external").attr(
		"title", function(i, val)
			{ return val + " (Link to an external website)"; }
	).attr('target', '_blank');

	// campl-teasers don't work correctly if title messed with
	$('.campl-teaser a').filter(function(){
		return(this.hostname &&
		this.hostname !== location.hostname &&
		!this.hostname.match(/\.cam\.ac\.uk$/gi));
	}).attr('target', '_blank');
